...
{
    $mysqli = new mysqli('localhost', 'root', '', 'Omnia_di2');
    $stmt = $mysqli->prepare("SELECT * FROM datetim WHERE MONTH(DATE) = ? AND YEAR(DATE) = ?");
    ...
    while ($currentDay <= $numberDays) {
        ...
        if ($date < date('Y-m-d')) {
            ...
            $calendar .= "<td class='$today'><h4>$currentDay</h4> <a href='/themes/calendriers'?date=" . $date . "' class='btn btn-success btn-xs'> <span class='glyphicon glyphicon-ok'></span> Book Now</a>";
        }
